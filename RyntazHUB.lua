-- AutoRob (Super Simplified) - V5.2 (ROB_ORDER Fix)
-- เน้นการปล้น, เทเลพอร์ต, และ Server Hop (ถ้าต้องการ) - UI น้อยที่สุด

-- (Services และ waitForCharacter, CharacterAdded เหมือนเดิม)
...
local function waitForCharacter()
    print("[AutoRob V5.2] Waiting for character...")
    local attempts = 0
    repeat
        attempts = attempts + 1
        if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Character:FindFirstChildOfClass("Humanoid") then
            ...
            if Humanoid and RootPart then print("[AutoRob V5.2] Character found."); return true end
        end
        task.wait(0.3)
    until attempts > 60
    print("[AutoRob V5.2] ERROR: Character not found after timeout."); return false
end
...
Player.CharacterAdded:Connect(function(newChar)
    ...
    print("[AutoRob V5.2] Character respawned/added.")
    if isRobbingGlobally then
        print("[AutoRob V5.2] Character died during robbery. Sequence may need manual restart or will attempt next heist if applicable.")
        if currentRobberyCoroutine then
            task.cancel(currentRobberyCoroutine)
            currentRobberyCoroutine = nil
            isRobbingGlobally = false
            print("[AutoRob V5.2] Active robbery coroutine cancelled due to character death.")
        end
    end
end)
...
local TARGET_HEISTS_TO_ROB_SEQUENCE = {"JewelryStore", "Bank", "Casino"} -- <<<< ใช้ตัวแปรนี้
...
-- ไม่ต้องมี ROB_ORDER แยกอีกต่อไป จะใช้ TARGET_HEISTS_TO_ROB_SEQUENCE โดยตรง

-- ... (ส่วน Helper Functions, attemptSingleRobberyAction, executeFullHeistRobbery, ServerHop เหมือนเดิมจาก V5.1) ...
...
local function attemptSingleRobberyAction(actionConfig, heistFolder)
    if not(Character and Humanoid and HumanoidRootPart and Humanoid.Health > 0) then print("[Robbery] Character dead or missing."); return false end
...
function executeAllRobberiesAndHop()
    ...
    print("--- Starting Full Auto-Robbery Sequence (V5.2) ---")
    local overallStartTime = tick()
    
    -- ใช้ TARGET_HEISTS_TO_ROB_SEQUENCE ที่ประกาศไว้ใน Config
    for i, heistName in ipairs(TARGET_HEISTS_TO_ROB_SEQUENCE) do 
        ...
        if heistConfig then
            ...
            if not executeFullHeistRobbery(heistName) then
                print(string.format("[AutoRob] Heist %s failed or was interrupted.", heistConfig.DisplayName or heistName))
            end
        else
...
local function createControlUI()
    ...
    local screenGui = Instance.new("ScreenGui",playerGui);screenGui.Name="RyntazAutoRobV5_Control";screenGui.ResetOnSpawn=false;screenGui.DisplayOrder=1000;screenGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
    local main=Instance.new("Frame",screenGui);main.Size=UDim2.new(0,220,0,80);main.Position=UDim2.new(0.01,0,0.5,-40);main.BackgroundColor3=Color3.fromRGB(25,25,30);main.BorderColor3=Color3.fromRGB(0,200,100);main.BorderSizePixel=1;local c=Instance.new("UICorner",main);c.CornerRadius=UDim.new(0,5);local l=Instance.new("UIListLayout",main);l.Padding=UDim.new(0,5);l.HorizontalAlignment=Enum.HorizontalAlignment.Center;l.VerticalAlignment=Enum.VerticalAlignment.Center;l.SortOrder=Enum.SortOrder.LayoutOrder
    local title=Instance.new("TextLabel",main);title.Name="Title";title.LayoutOrder=1;title.Size=UDim2.new(1,-10,0,20);title.Text="Ryntaz AutoRob V5.2";title.TextColor3=Color3.fromRGB(0,255,120);title.Font=Enum.Font.Michroma;title.TextSize=12;title.BackgroundTransparency=1
...
if waitForCharacter() then
    pcall(createControlUI)
    local autoStartRobbery = true 
    if autoStartRobbery then
        print("[AutoRob V5.2] Auto-start. Waiting 3s...")
        task.wait(3)
        if not isRobbingGlobally then task.spawn(executeAllRobberiesAndHop) end
    else
        print("[AutoRob V5.2] Auto-start disabled. Click button or call executeAllRobberiesAndHop()")
    end
else
    print("[AutoRob V5.2] Script terminated: Character not loaded.")
end
