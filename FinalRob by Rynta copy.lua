...
function MainUI:Build()
    ...
    MainUI.ScreenGui = UILib.Create("ScreenGui"){ Name = "RyntazHubTitanV3_4", Parent = Player:WaitForChild("PlayerGui"), ZIndexBehavior = Enum.ZIndexBehavior.Sibling, ResetOnSpawn = false, DisplayOrder = 1001 }
    ...
    local introAppearTween = UILib.Tween(introText, { Size = UDim2.new(0.8,0,0.2,0), TextSize = 80, Transparency = 0, Rotation = 0 }, { Time = THEME.Animation.Intro, EasingStyle = Enum.EasingStyle.Elastic, EasingDirection = Enum.EasingDirection.Out })
    
    if introAppearTween then introAppearTween.Completed:Wait() else task.wait(THEME.Animation.Intro) end
    task.wait(0.7) 
    
    local introFadeOutTween = UILib.Tween(introText, { Transparency = 1, Position = UDim2.new(0.5,0,0.6,0) }, { Time = THEME.Animation.Medium })
    if introFadeOutTween then introFadeOutTween.Completed:Wait() else task.wait(THEME.Animation.Medium) end
    introFrame:Destroy()
    ...
    local titleText = UILib.Create("TextLabel"){ Parent = titleBar, Name = "Title", Size = UDim2.new(1, -80, 1, 0), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5,0.5), BackgroundTransparency = 1, Font = THEME.Font.Title, Text = "RYNTAZHUB :: TITAN", TextColor3 = THEME.Colors.AccentBright, TextSize = 18, TextXAlignment = Enum.TextXAlignment.Center }
    
    local buttonSize = UDim2.new(0,12,0,12); local btnY=0.5; local btnYO=-6
    local closeBtn = UILib.Create("ImageButton"){Parent=titleBar,Name="Close",Size=buttonSize,Position=UDim2.new(0,10,btnY,btnYO),Image="rbxassetid://13516625",ImageColor3=THEME.Colors.CloseButton,BackgroundTransparency=1,ZIndex=4}
    local minBtn = UILib.Create("ImageButton"){Parent=titleBar,Name="Minimize",Size=buttonSize,Position=UDim2.new(0,30,btnY,btnYO),Image="rbxassetid://13516625",ImageColor3=THEME.Colors.MinimizeButton,BackgroundTransparency=1,ZIndex=4}
    local maxBtn = UILib.Create("ImageButton"){Parent=titleBar,Name="Maximize",Size=buttonSize,Position=UDim2.new(0,50,btnY,btnYO),Image="rbxassetid://13516625",ImageColor3=THEME.Colors.MaximizeButton,BackgroundTransparency=1,ZIndex=4}
...
RyntazHub.UIController.PopulateHeists=function()local heistsPage=RyntazHub.UIHooks.HeistsPage;if not heistsPage then liveDebug("PopulateHeists: HeistsPage is nil"); return end;for _,child in ipairs(heistsPage:GetChildren())do if child.Name=="HeistCard"then child:Destroy()end end;local count=0;for key,heistData in pairs(RyntazHub.Data.Analyzed)do if heistData and #heistData.AnalyzedSequence>0 then MainUI:CreateHeistCard(heistsPage,heistData,RyntazHub.ExecutionEngine.Run);count=count+1 end end;if count==0 then UILib.Create("TextLabel"){Parent=heistsPage,Text="ไม่พบ Heist ที่สามารถดำเนินการได้ โปรดตรวจสอบ Config หรือการสแกน",Font=THEME.Font.UI,TextSize=14,TextColor3=THEME.Colors.Warning,Size=UDim2.new(1,-20,0,50),BackgroundTransparency=1,TextWrapped=true,LayoutOrder=1}end;liveDebug("PopulateHeists: Finished.")end
